    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks =
                sites.join(ranks).flatMapToPair(kv -> {
                    ...
                    Iterator<Integer> edgeIterator = kv._2()._1().edgeIterator();
                    while (edgeIterator.hasNext()) {
                        final Integer destId = edgeIterator.next();
                        contributions.add(new Tuple2(destId, rank / (double) website.getNEdges()));
                    }
                    return contributions;
                });
        return newRanks
                .reduceByKey((Double r1, Double r2) -> r1 + r2)
                .mapValues((Double r) -> 0.15 + 0.85 * r);
    }
}
