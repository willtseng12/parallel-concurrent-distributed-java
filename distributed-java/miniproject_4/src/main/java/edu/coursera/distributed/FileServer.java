...
//            throw new UnsupportedOperationException();
            ...
            Thread thread = new Thread(() -> {
                try {
                    ...
                    String pathString = line.split(" ")[1];
                    PCDPPath path = new PCDPPath(pathString);
                    String fileContent = fs.readFile(path);

                    if (fileContent != null) {
                        serverContent(printWriter, fileContent);
                    } else {
                        reply404Http(printWriter);
                    }
                    printWriter.close();
    ...
    private void reply404Http(PrintWriter writer) {
        writer.write("HTTP/1.0 404 Not Found\r\n");
        writer.write("Server: FileServer\r\n");
        writer.write("\r\n");
    }

    private void serverContent(PrintWriter writer, String content) {
