...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        while ((n = nodesLoaded.poll()) != null) {
            ...
            if (!n.lock.tryLock()) { // some other threada must be working on this node
                continue;
            }

            if (n.isDead) { // node already merged
                n.lock.unlock();
                continue;
            }

            Edge<ParComponent> minEdge = n.getMinEdge();
            if (minEdge == null) { // all nodes are merged, finished
            ...
            if (!other.lock.tryLock()) { // other node is currently being processed
                nodesLoaded.add(n);
                n.lock.unlock();
                continue;
            }
